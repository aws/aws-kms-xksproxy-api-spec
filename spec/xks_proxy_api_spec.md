...
Each type of HSM will require a corresponding XKS Proxy but a single XKS Proxy instance can serve a cluster of HSMs. 
...
Refer to [Appendix C](#appendix-c) for the CDIV computation.
...
implementation guidelines described in [Appendix C](#appendix-c).
  ...
  \begin{center}
    \begin{tabular}{l l l}
     ...
     a malformed response   & (Create/UpdateCustomKeyStore)       & response from the external \\
                            ...
                            & (Cryptographic operations)          & proxy vendor. \\
      \hline
...
Customers use the KMS CreateCustomKeyStore API to create a custom key store of type external key store. As part of this API call, customers specify the XKS Proxy API endpoint and a 256-bit secret (authentication key). KMS uses the authentication-key to sign all XKS Proxy API requests using [AWS SigV4](https://docs.aws.amazon.com/general/latest/gr/signature-version-4.html). AWS SigV4 is already in wide use and supported by a number of third-party API clients like [Postman](https://www.postman.com/), [Insomnia](https://insomnia.rest/) and [Paw.](https://paw.cloud/)  KMS uses the authentication key as the `secret-access-key` in [calculating the signature](https://docs.aws.amazon.com/general/latest/gr/sigv4-calculate-signature.html) to authenticate itself to the XKS Proxy. The same secret is also configured by the customer into the XKS Proxy so the proxy can verify the signature independent of AWS authentication and authorization. Anyone who possesses this secret can make successful API calls to the XKS Proxy. The XKS Proxy administrator must ensure that this secret is not exposed to unauthorized users. Instead of storing the secret as plaintext in an XKS Proxy configuration file, it is preferable to store it as ciphertext and use a key in the external key manager to decrypt the secret when the proxy starts up. [Appendix A](#appendix-a) describes SigV4 usage for signing requests from the XKS Proxy Management Fleet to the XKS Proxy.
...
The sample bash script and its output in [Appendix D](#appendix-d) illustrate how `curl` can be used to make an XKS API call and measure the time spent in different phases of the interaction. Refer to the `curl` [man page](https://man7.org/linux/man-pages/man1/curl.1.html) for details.


\pagebreak

## Appendix A: SigV4 Signatures on XKS Proxy Requests {#appendix-a}
...
## Appendix C: CDIV Implementation Guidelines {#appendix-c}
...
## Appendix D: Using curl for XKS API calls {#appendix-d}
