...
7. **ekmFleetDetails.healthStatus** - Status of health check on the HSM from XKS proxy. The possible statuses are `ACTIVE`, `DEGRADED` and `UNAVAILABLE`. `ACTIVE` means that HSM is healthy, `DEGRADED` means that HSM is unhealthy but can still serve traffic and `UNAVAILABLE` means that HSM is unable to serve traffic.
